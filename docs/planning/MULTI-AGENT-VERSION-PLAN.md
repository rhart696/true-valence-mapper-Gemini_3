...
Create the following repos on GitHub (all under `rhart696`), each initialized from the current Gemini_3 build commit so they share the same baseline:

| Edition | Repo Name | Primary Goal | Kickoff Tasks |
| --- | --- | --- | --- |
| Gemini_3 Code *(current repo)* | `true-valence-mapper-gemini_3` | Preserve reference implementation shipped with Gemini_3 Code. | Rename repo locally & remotely, keep docs updated, act as baseline for comparisons. |
| Codex | `true-valence-mapper-codex` | Showcase GPT-5/Codex automation, richer developer tooling. | Fork from Gemini_3 edition, swap orchestration scripts to Codex CLI, document agent differences. |
...
| Gemini_3 | `true-valence-mapper-gemini_3` | Optimize for Google Gemini_3 2.5 workflows and AI Studio deployment. | Align prompts to Gemini_3, add API keys (store in 1Password), document latency/cost metrics. |
...
For each edition (except Gemini_3, already initialized):
1. `gh repo create rhart696/<repo-name> --public --template rhart696/true-valence-mapper-gemini_3` *(or manually clone and push if template unavailable).*  
...
## Migration Tasks for the Current (Gemini_3) Repository

1. Rename the repo locally to `true-valence-mapper-gemini_3` (`git remote set-url origin git@github.com:rhart696/true-valence-mapper-gemini_3.git` after the remote is created).
2. Update documentation (done in this commit) to use the new full/short names and note the Gemini_3 edition.
...
3. **Trigger the workflow**:  
   ...
   - The job runs `scripts/setup-multi-agent-repos.sh`, which uses `gh` to create the parent repo plus Gemini_3, Codex, Multi Path, Gemini_3, BMAD-Method, Stitch, and Speckit repositories if they don’t already exist.
4. **Verify in GitHub**:  
   - Each repo will appear under `github.com/rhart696/`.  
   - Rename this repo to `true-valence-mapper-gemini_3` (GitHub → Settings → General) so the template name matches the workflow assumptions.
...
2. Rename/push the current repo as `true-valence-mapper-gemini_3`.
3. Spin up the Codex edition next (prioritize since Codex agent is active), followed by Multi Path and Gemini_3.
